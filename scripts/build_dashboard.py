...
JOBLINE_MEASURES = {"pocet jobline", "jobline", "job lines", "storejoblines", "store job lines"}
...
def is_measure_label(value: Any, labels: set[str]) -> bool:
    return normalize_text(value) in {normalize_text(label) for label in labels}
...
def find_pivot_measure_row(worksheet: Any, date_row: int | None, header_row: int) -> int | None:
    if date_row is None:
        return None
    candidate = date_row + 1
    if candidate > header_row:
        return None
    labels = [
        normalize_text(worksheet.cell(candidate, column_index).value)
        for column_index in range(2, worksheet.max_column + 1)
    ]
    if any(label in {normalize_text(value) for value in JOBLINE_MEASURES | PIECE_MEASURES} for label in labels):
        return candidate
    return None


def build_pivot_context(worksheet: Any, row_index: int) -> dict[str, str]:
...
def parse_pivot_sheet(worksheet: Any, sheet_name: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row_index in range((measure_row or header_row) + 1, worksheet.max_row + 1):
        context = build_pivot_context(worksheet, row_index)
        label = context["label"]
        ...
        indent = int(context["indent"])
        upper_label = context["upper_label"]

        row_has_values = any(
            safe_number(worksheet.cell(row_index, column_index).value, default=0) > 0
            for column_index in range(2, worksheet.max_column + 1)
        )
        ...
        for column_index in range(2, worksheet.max_column + 1):
            ...
            if date_value is None:
                continue
            measure_label = normalize_text(worksheet.cell(measure_row, column_index).value) if measure_row is not None else ""
            ...
            if measure_row is not None and measure_label in {normalize_text(value) for value in PIECE_MEASURES}:
...
<body>
  ...
  <script>
    ...
    function balikovkaPieceCount(rows, sheetName) {
      const allowed = balikovkaTransportSet(sheetName);
      if (!allowed.size) return 0;
      return rows.reduce((sum, row) => {
        const transport = normalizeText(row.doprava);
        return sum + (allowed.has(transport) ? (Number(row.piece_count) || 0) : 0);
      }, 0);
    }

    function totalPieceCount(rows) {
    ...
    function renderDashboard(sheetName, index) {
      ...
      root.innerHTML = `
        ...
        <section class="cards">
          ...
          <div class="card">
            <span>Počet kusov balíkovky</span>
            <strong id="${prefix}kpiBalikovkaPieces">0</strong>
            <small id="${prefix}kpiBalikovkaPiecesShare">0 % z kusov</small>
            <small>Target: &lt; 10 % of kusov</small>
          </div>
      ...
      function renderKpis(rows) {
        ...
        const totalPieces = totalPieceCount(rows);
        const balikovkaPieces = balikovkaPieceCount(rows, sheetName);
        const balikovkaPiecesRatio = totalPieces ? balikovkaPieces / totalPieces : 0;
        ...
        document.getElementById(`${prefix}kpiBalikovkaPieces`).textContent = formatInt(balikovkaPieces);
        document.getElementById(`${prefix}kpiBalikovkaPieces`).className = 'ratio ' + balikovkaClass(balikovkaPiecesRatio);
        document.getElementById(`${prefix}kpiBalikovkaPiecesShare`).textContent = `${formatPct(balikovkaPiecesRatio)} z kusov`;
